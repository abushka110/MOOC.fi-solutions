...
def askPoint():
    ...
    while exam_Exercices != "":
        space_index = exam_Exercices.find(" ")
        exam_Points.append(int(exam_Exercices[:space_index]))
        exercices_Points.append(int(exam_Exercices[space_index + 1:]))
        exam_Exercices = input("Exam points and exercices completed: ")

    total_points = points_total(exam_Points, exercices_Points)
    grades_list = points_to_grades(total_points)
    print("Statistics:")
    print(f"Points average: {points_average(exam_Points, exercices_Points):.1f}")
    print(f"Pass percentage: {pass_percentage(grades_list):.1f}")
...
# 
def points_average(exam_points: list, exercices_points: list):
    # convert exercices points to granted points
    granted_points = []
    for item in exercices_points:
        item = item / 10
    ...
    return sum(exam_plus_granted) / len(exam_plus_granted) 
# 
